...
public class AccountController {
	...
	public AccountController(PendingAccountsManager pendingAccounts, AccountsManager accounts,
			RateLimiters rateLimiters, MessagesManager messagesManager, TimeProvider timeProvider,
	...
	public Response createAccount(@PathParam("transport") String transport, @PathParam("number") String number,
		...
		if (testDevices.containsKey(number)) {
			...
			// smsSender.deliverSmsVerification(number, client,
	...
	public AuthorizationToken verifyToken(@Auth Account account) throws RateLimitExceededException {
		if (!tokenGenerator.isPresent()) {
			logger.debug("Attempt to authorize with key but not configured...");
	...
	public void setGcmRegistrationId(@Auth Account account, @Valid GcmRegistrationId registrationId) {
		...
		if (registrationId.isWebSocketChannel())
