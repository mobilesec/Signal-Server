...
public class FederationControllerV1 extends FederationController {
	...
	public FederationControllerV1(AccountsManager accounts, MessageController messageController) {
	...
	public void sendMessages(@Auth FederatedPeer peer, @PathParam("source") String source,
			@PathParam("sourceDeviceId") long sourceDeviceId, @PathParam("destination") String destination,
			@Valid IncomingMessageList messages) throws IOException {
		try {
	...
	@Produces(MediaType.APPLICATION_JSON)
	public ClientContacts getUserTokens(@Auth FederatedPeer peer, @PathParam("offset") int offset) {
